...
export const getBellPosition = Fn(([ phase, zenith, azimuth, bottomFactor = 0 ]) => {
    ...
    const contraction = sin(modifiedPhase).mul(0.5).add(0.5).toVar();
    const xr = contraction.mul(-0.13).add(1.22).toVar();

    const riffles = mix(1.0, sin(azimuth * 16.0 + 0.5*Math.PI) * 0.02 + 1.0, smoothstep(0.5,1.0,zenith));

    xr.mulAssign(riffles);
    const yr = contraction.mul(0.18).add(1.04);
    const polarAngle = modifiedZenith * Math.PI * 0.52;
