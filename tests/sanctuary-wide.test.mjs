...
import {colonyLayout,wideLifePoints} from '../src/scene/sanctuary/VentLife.js';
...
test('M6.4 preserves macro geometry, dynamics, plume, optics, animals, camera, UI and idle source',()=>{
 ...
 allowed.push('src/scene/HeroScene.jsx'); // M6.6.1 input validation only; exact delta tested separately.
 assert.ok(changed.filter(f=>!m7Paths.includes(f)).every(f=>allowed.includes(f)),changed.join('\n'));
 ...
 assert.equal(omitShelf(read('src/scene/sanctuary/geology.js')),omitShelf(old('src/scene/sanctuary/geology.js')));
...
test('wide biological accents reuse the exact existing sites and 40 horizontal positions',()=>{
 ...
 // Retain the historical M6.3 utility contract; M6.4's finished-surface
 // attachment has its own tests instead of pretending these old radii persist.
 const s=new Sanctuary(new Scene()),wide=wideLifePoints(c.points,s.solids);
