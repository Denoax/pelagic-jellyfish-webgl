...
const[base='http://127.0.0.1:5209/',label='after',only='']=process.argv.slice(2);
for(const mode of ['stationary','ravine','life','illumination']){
 if(only&&only!==mode)continue;
 const b=await browserSession(`../m6-5-evidence/motion/${label}-${mode}`);
 try{
