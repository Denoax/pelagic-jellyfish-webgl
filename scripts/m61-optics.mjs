...
import {ORIFICE,DIFFUSE} from '../src/scene/sanctuary/VentDynamics.js';
const [base='http://127.0.0.1:5201/',label='shimmer-optics',motion='0',source='primary']=process.argv.slice(2);
const vent=source==='diffuse'?DIFFUSE[0]:ORIFICE;
const height=source==='diffuse'?vent.y+.48:-2.1;
const cameraX=source==='diffuse'?vent.x+2:vent.x;
const cameraZ=source==='diffuse'?vent.z+2:-25.7;
const targetZ=source==='diffuse'?vent.z:-23.95;
...
try{
 ...
 await b.ev(`window.__CAMERA_LAB__.select('explore');const c=window.__CAMERA_LAB__.camera;c.position.set(${cameraX},${height},${cameraZ});c.lookAt(${vent.x},${height},${targetZ});c.updateMatrixWorld()`);
