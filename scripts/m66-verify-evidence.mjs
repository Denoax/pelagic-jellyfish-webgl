...
const root='/home/mani/dev/jellyfish-studio/m6-6-evidence';
const read=p=>JSON.parse(readFileSync(p));
const rows=[];
for(const key of ['explore','stacked','ravine-edge','low','low-clear','high','documentary','wide','chimney','grain','glow-close','deep','drift','ravine','base','colony','shelf','spires','illumination','portrait']){
...
const clips=[];
for(const side of ['before','after'])for(const mode of ['orbit','parallel','ravine','life','stationary']){
...
if(rows.some(x=>!x.cameraEqual||!x.buffersEqual||x.baselineErrors||x.candidateErrors||Math.abs(x.animalTime[0]-x.animalTime[1])>1e-7||Math.abs(x.phase[0]-x.phase[1])>1e-7||Math.abs(x.plumeTime[0]-x.plumeTime[1])>1e-7))process.exitCode=1;
