...
const root='/home/mani/dev/jellyfish-studio/m6-5-evidence';
const read=p=>JSON.parse(readFileSync(p));
const rows=[];
for(const key of ['explore','wide','chimney','grain','glow-close','deep','drift','ravine','base','colony','shelf','spires','illumination','portrait']){
...
const clips=[];
for(const side of ['before','after'])for(const mode of ['stationary','ravine','life','illumination']){
