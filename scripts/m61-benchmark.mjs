...
for(const[name,p]of cases){
 ...
 if(name==='close')pose=`c.select('explore');c.camera.position.set(-3,-2.1,-25.7);c.camera.lookAt(-3,-2.1,-24);c.camera.updateMatrixWorld();`;
 ...
 const r=spawnSync(process.execPath,['scripts/specimen-evidence.mjs',url,`${label}-${name}`,'perf','30'],{stdio:'inherit',env:{...process.env,EVIDENCE_ROOT:process.env.EVIDENCE_ROOT||'/home/mani/dev/jellyfish-studio/m6-1-evidence/performance',EVIDENCE_SOURCE_REV:sha,EVIDENCE_SOURCE_ROOT:source,EVIDENCE_EVAL:code}});
