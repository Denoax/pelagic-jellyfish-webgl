...
const[base='http://127.0.0.1:5211/',label='after',only='']=process.argv.slice(2);
for(const mode of ['orbit','parallel','ravine','life','stationary']){
 if(only&&only!==mode)continue;
 const b=await browserSession(`../m6-6-evidence/motion/${label}-${mode}`);
 try{
  ...
  if(['orbit','parallel','ravine','life'].includes(mode))await b.ev(`(()=>{const c=window.__CAMERA_LAB__;c.select('explore');const start=performance.now();function tick(t){const u=${mode==='life'?'1-':''}Math.min(1,(t-start)/20000);${mode==='orbit'?`const a=.7+u*.95;c.camera.position.set(-6+Math.sin(a)*9,-10.8,-22+Math.cos(a)*9);c.camera.lookAt(-7,-12.8,-22);` :mode==='parallel'?`c.camera.position.set(.5,-11.8,-13-u*16);c.camera.lookAt(-8,-13,-19-u*14);`:mode==='ravine'?`c.camera.position.set(4+Math.sin(u*2)*1.2,-11.6,-13-u*21);c.camera.lookAt(2.8,-15,-22-u*18);`:`c.camera.position.set(1+u*13,-11+u*7,-15+u*7);c.camera.lookAt(-.3-u*2.1,-15+u*6,-22.6-u*4.4);`}c.camera.updateMatrixWorld();requestAnimationFrame(tick)}requestAnimationFrame(tick)})()`);
